...
impl PyBridge {
    ...
    pub fn call_function(&self, module: &str, function: &str, args: &[PyValue]) -> Result<PyValue, PyBridgeError> {
        ...
        Python::with_gil(|py| {
            // 导入模块
            let py_module = PyModuleType::import_bound(py, module)
                .map_err(|e| PyBridgeError::PythonError(format!("Failed to import module '{}': {}", module, e)))?;

            // 获取函数
            let py_func = py_module
                .getattr(function)
                .map_err(|e| PyBridgeError::PythonError(format!("Failed to get function '{}': {}", function, e)))?;
            ...
            let result = py_func
                .call1(py_tuple)
                .map_err(|e| PyBridgeError::PythonError(format!("Failed to call function '{}': {}", function, e)))?;
    ...
    pub fn execute_code(&self, code: &str) -> Result<PyValue, PyBridgeError> {
        ...
        Python::with_gil(|py| {
            ...
            match py.run_bound(code, None, None) {
                ...
                Err(e) => Err(PyBridgeError::PythonError(format!("Python execution error: {}", e))),
            }
        })
    }
    ...
    pub fn eval(&self, expression: &str) -> Result<PyValue, PyBridgeError> {
        ...
        Python::with_gil(|py| {
            match py.eval_bound(expression, None, None) {
                ...
                Err(e) => Err(PyBridgeError::PythonError(format!("Python eval error: {}", e))),
            }
        })
    }
}
...
fn py_value_to_pyobject_bound<'py>(py: Python<'py>, value: &PyValue) -> Result<Bound<'py, PyAny>, PyBridgeError> {
    match value {
...
fn pyobject_to_py_value_bound<'py>(py: Python<'py>, obj: &Bound<'py, PyAny>) -> Result<PyValue, PyBridgeError> {
    // 尝试提取为各种类型
    ...
    if let Ok(py_dict) = obj.downcast::<PyDict>() {
        ...
        for (key, value) in py_dict.iter() {
            let key_str = key.extract::<String>()
                .map_err(|_| PyBridgeError::TypeConversionError("Dict key must be string".to_string()))?;
